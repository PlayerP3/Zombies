...
class Paused(State):
    ...
    def enter(self):
       ...
       # but for now splash is selection screen
        ...
        self.parent_node.hud.activate_hud_elements(['Pause'])
        self.parent_node.hud.deactivate_hud_elements(['PlayerHealth','RoundNumber','Ammo','Points','Splash'])
        self.parent_node.player.movementx[0] = 0 
        self.parent_node.player.movementy[0] = 0 
        self.parent_node.player.movementx[1] = 0 
        self.parent_node.player.movementy[1] = 0 
        self.parent_node.player.is_interacting = False
        self.parent_node.player.weapon.state.emit('IDLE')

...
class Unplaced(State):
            ...
            for game_object in self.parent_node.surrounding_game_objects: 
                ...
                if collision:
                    ...
                    if game_object.is_interacting:
                        ...
                        # set interacting obj if not holding the weapon to be upgraded the continue
                        if self.parent_node.buildableData[self.parent_node.buildableObject]['upgradesFrom'] == game_object.inventory.inventory['weapons'][0]:                
    ...
    def handle_collision(self,game_object:object,axis:str):
        ...
        # if upgrades from is not being held currently
        if self.parent_node.buildableData[self.parent_node.buildableObject]['upgradesFrom'] != game_object.inventory.inventory['weapons'][0]:  
            return              

        # if what you upgrade from is in the player weapons
        elif self.parent_node.buildableData[self.parent_node.buildableObject]['upgradesFrom'] == game_object.inventory.inventory['weapons'][0]:
        
            # set display message and item image path
            self.parent_node.display_message.update_message(f"Hold E to interact")
            self.parent_node.buildableSprite.img_path = self.parent_node.buildableData[self.parent_node.buildableData[self.parent_node.buildableObject]['upgradesFrom']]['img_path']
    ...
    def end_condition(self):
        ...
        if self.parent_node.interactTimer.timer_complete:
            ...
            # if upgrade then remove weapon from player invenotry
            if self.parent_node.can_upgrade:
                remove_weapon(self.parent_node.interactingObj)

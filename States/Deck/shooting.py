...
from weapon import OnShotEffectMgr
import sys
...
class Shooting(State):
    ...
    def enter(self):
        ...
        self.parent_node.deck.shooting_timer.timer_init()
        self.parent_node.deck.shooting_timer.timer_complete = True
    ...
    def update(self):
        ...
        self.parent_node.deck.can_shot_be_fired2()

        # if there is a projectile in the queue
        if self.parent_node.deck.projectile_queue:

            to_remove = []

            # go through each bullet in the projectile queue
            for bullet in self.parent_node.deck.projectile_queue:
            ...
            if to_remove:
                ...
                for blt in to_remove:
                    ...
                    self.parent_node.deck.projectile_queue.remove(blt)

            # if empty start burst countdown to prevent shooting
            if not self.parent_node.deck.projectile_queue:
        ...
        if self.parent_node.deck.bullets_remaining_in_mag == 0: # here bullets being 0 means that all has been fired
    ...
    def completed(self):
        ...
        self.parent_node.deck.projectile_queue = []
        self.done = False
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_r:
                ...
                if not self.parent_node.deck.is_dual_wield:
                    ...
                    if (self.parent_node.deck.bullets_remaining_in_mag < self.parent_node.deck.magazine_size):

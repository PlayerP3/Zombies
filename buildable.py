...
from States.Bench.unplaced import Unplaced
...
class Bench(Interactable):
    ...
    def __init__(self,partsNeeded:int=3,buildableObject:str='',canBeReplaced:bool=False,buildingTimer:float=1.5,pickupTimer:float=0.2,replaceTimer:float=0.2,
        ...
        # whether it is an upgrade bench
        ...
        self.stateInteractTimeLimit = {'UNPLACED':0.3,
                                       'IDLE':self.buildingTimer,
                                       'BUILDABLEBUILT':self.pickupTimer,
                                       'BUILDABLETAKEN':self.replaceTimer}
        
        self.stateTimeLimit = {'UNPLACED':1,
                                'IDLE':1,
                                'BUILDABLEBUILT':1,
                                'BUILDABLETAKEN':1}
    ...
    def init(self):
        ...
        # enter different state depending on if its an upgrade bench or not
        if self.can_upgrade:
            self.state = self.states['UNPLACED']

        elif not self.can_upgrade:
